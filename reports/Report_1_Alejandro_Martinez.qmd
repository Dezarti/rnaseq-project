---
title: "PART 1: Analysis of a small data-set."
...
The present work analyses the fictional data-set 'DATA_SET_REFERENCE_EVAL.csv', which presents 400 measurements of HDL and LDL in patients across two planets (Earth and Venus) from two populations (tall and small), as seen in @tbl-summary-py.

The measurement units were not given, but based on the magnitude of the numbers, HDL and LDL are assumed to be in milligrams per deciliter (mg/dL). No further information was given about the dataset due to it being fictional; nonetheless, in a real one, the source must be trustworthy with traceable methods.
...
Outliers were identified using a threshold of three standard deviations from the mean. For each numerical column in the dataset, we calculated the upper limit (@eq-outlier_1) and lower limit (@eq-outlier_2) as follows:
...
### (a) 1 analysis of 2 continuous variables
...
### (b) 1 analysis of 1 continuous variable vs 1 categorical variable.
